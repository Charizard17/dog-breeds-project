<?php
    ...
    for ($j = 0; $j < count($dogBreedArray); ++$j) {
        if (strpos($dogBreedArray[$j]["weight"]["metric"], " - ")) {
            ...
            $dogBreedArray[$j]["minWeight"] = $dogBreedArray[$j]["weight"]["metric"];
            $dogBreedArray[$j]["maxWeight"] = $dogBreedArray[$j]["weight"]["metric"];
            $dogBreedArray[$j]["weight"]['metric'] = $dogBreedArray[$j]["minWeight"] ." - ". $dogBreedArray[$j]["maxWeight"];
        }
    }
    ...
    for ($k = 0; $k < count($dogBreedArray); ++$k) {
        if (strpos($dogBreedArray[$k]["height"]["metric"], " - ")) {
            ...
            $dogBreedArray[$k]["minHeight"] = $dogBreedArray[$k]["height"]["metric"];
            $dogBreedArray[$k]["maxHeight"] = $dogBreedArray[$k]["height"]["metric"];
            $dogBreedArray[$k]["height"]['metric'] = $dogBreedArray[$k]["minHeight"] ." - ". $dogBreedArray[$k]["maxHeight"];
        }
    }
